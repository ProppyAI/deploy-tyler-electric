...
# lib/autopilot/ — tracked .py + .json (durable hardened allowlist + merge helper).
# These propagate to tenants so `harness autopilot harden` and the allowlist merge
# work everywhere; the top-level lib glob above intentionally skips subdirs.
for f in sorted(ls_files("lib/autopilot/*")):
    if os.path.isfile(f) and f.split(".")[-1] in ("py", "json"):
        files.append({"src": f, "dst": f})
...
user_files = [
    ...
    {
        "src": ".claude/hooks/autopilot-usage-tally.sh",
        "dst": "~/.claude/hooks/autopilot-usage-tally.sh",
        "mode": "0755",
        "_note": "Installed by harness-setup-dev — PostToolUse usage tally for the approval-hardening flywheel",
    },
]
