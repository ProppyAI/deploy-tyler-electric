...
' "$json")"
  ...
  url="${url%/}"
  # Allow https:// (any host) or an EXACT localhost/127.0.0.1 loopback base with an
  # optional NUMERIC port (1-5 digits). Anchored + bounded digit-only port closes the
  # prefix-glob bypass (http://localhost.evil.com) AND the RFC-3986 userinfo trick
  # (http://localhost:@evil.com parses host=evil.com → token would be forwarded).
  local _loopback_re='^http://(localhost|127\.0\.0\.1)(:[0-9]{1,5})?$'
  if [[ "$url" != https://* && ! "$url" =~ $_loopback_re ]]; then
...
' "$row_id" "$apply")"
  ...
  local response
  # --max-redirs 0: curl does not follow redirects without -L, but make it
  # explicit so a 3xx can never forward the Bearer token to another host.
